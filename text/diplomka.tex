...
Podmínky, která jsou na výsledné rozdělení grafu kladeny se mohou lišit v závislosti na daném použití. V této kapitole popíšeme klasická kritéria, která se používají pro určování kvality rozdělení, v kapitole \ref{dalsiParametry} poté bude popsán problém dělení grafů vzhledem k dodatečným kritériím (například vzhledem k maticovým operacím) a s ohledem na více kritérií.
...
\end{enumerate}
\textbf{Rozdělení grafu} běžně zapisujeme jako vektor $P$ o délce $n$ takový, že pro každý vrchol $v \in V$ je podgraf, v němž se vrchol v nachází, určen $v$-tou složkou vektoru $P$.

Pokud má graf $G$ sudý počet vrcholů a rozdělíme ho na dvě části s množinami vrcholů $V_1, V_2$, které mají stejný počet prvků, nazveme toto rozdělení \textbf{bisekcí} a velikost hranového separátoru nazýváme šířkou bisekce.

Mějme graf $G=(V,E)$ a jeho rozdělení na $k$ podgrafů s množinami vrcholů $V_1, V_2, \ldots, V_k$. Množinu hran, jejichž jeden koncový bod náleží do $V_i$ a druhý do $V_j$ pro $i \neq j$ nazveme \textbf{hranovým separátorem}, značíme $\delta(A,B)$. 
\textbf{Vrcholovým separátorem} $V_S$ grafu $G$ nazveme podmnožinu množiny vrcholů $V$ takovou, že odstraněním všech vrcholů náležících do této množiny z grafu $G$ dojde k jeho rozdělení na $k$ podgrafů, mezi nimiž nevede ani jedna hrana. Řešení problému optimální transformace mezi hranovým a vrcholovým separátorem, známe-li jeden z nich, můžeme naleznout v \cite{pofa:90}.

Ukazuje se, že rozhodovací problém pro optimální rozdělení grafu je NP-úplný \cite{gajo:79}. Existují však algoritmy, které rozdělí graf v rozumném čase, přičemž kvalita jimi nalezeného rozdělení bude poměrně dobrá \cite{lita:79}. To nám ale nic neříká o řešení problému samotného nalezení optimálního hranového nebo vrcholového separátoru \cite{liu:89}

\section{Obecné schéma víceúrovňového dělení grafů}
\label{multilevel}

Víceúrovňové dělení grafů slouží k převedení problému dělení grafu s velkým počtem vrcholů na problém dělení grafu s počtem vrcholů výrazně menším. Tím může dojít k výraznému zkrácení času potřebného pro běh algoritmu \TODO{zdroj}, proto je tento postup využíván v profesionálních softwarových nástrojích pro dělení grafů. Nejobecnější popis tohoto schématu sestává ze tří základních fází: zhrubovací fáze (coarsening phase), rozdělení vzniklého grafu a projekce tohoto rozdělení zpět na původní graf (refinement phase).

\subsection{Zhrubovací fáze}
Mějme graf $G_0=(V_0,E_0)$. Pak ve zhrubovací fázi dochází k vytváření posloupnosti grafů $G_1,...G_m$ takových, že každý z těchto grafů je vždy faktorgrafem předchozího a má méně vrcholů než předchozí. Pro udržení informací o původním grafu je nutné, abychom ohodnocení vrcholů a hran při tvorbě faktorgrafu položili pro každý vrchol, resp. hranu roven součtu vah všech vrcholů, resp. hran, spojením kterých tento vrchol, resp. hrana vznikl.

Pro tvorbu faktorgrafu daného grafu se využívá principu nalezení maximálního párování grafu a následného spojení koncových vrcholů hran náležících do párování \cite{edmo:65,lawl:76}.

\subsection{Dělení získaného grafu}
V této fázi dochází k rozdělení grafu $G_m=(V_m,E_m)$ libovolným algoritmem pro dělení grafů. Označme získané rozdělení $P_m$.

\subsection{Zjemňovací fáze}
Cílem zjemňovací fáze je promítnout rozdělení $P_m$ grafu $G_m$ na rozdělení $P_0$ vstupního grafu $G_0$ přes rozdělení $P_{m-1},...,P_1$ grafů $G_{m-1},...,G_1$. Rozdělení grafu $G_i$ lze vytvořit z rozdělení grafu $G_{i+1}$ tak, že všechny vrcholy grafu $G_i$, jejichž spojením vznikl multivrchol $v$ grafu $G_{i+1}$ přiřadíme do části rozdělení, v níž leží vrchol $v$.

Tímto postupem však může dojít ke ztrátě optimality rozdělení. Proto je vhodné v každém kroku zjemňovací fáze zapojit nějaký vylepšovací algoritmus. Většinou se tento vylepšovací algoritmus neaplikuje na celý graf, ale pouze na části jednotlivých podgrafů ležící v blízkosti hranového separátoru. Většina vylepšovacích algoritmů je založena na KL algoritmu.
...
\TODO{doplnit, neuplne asi ani ne}
...
Nejprve se omezme na ireducibilní, pozitivně definitní, symetrickou matici $A_T$ o rozměrech $n \times n$, jejíž přidružený graf $G(A_T)$ je strom. V tomto případě je $A_T$ tzv. perfektní eliminační matice, tj. existuje permutační matice $P$ taková, že Choleského rozklad matice $PA_TP^T$ nebude obsahovat žádné zaplnění \cite{rose:72} (Matici $PA_TP^T$ můžeme vnímat pouze jako přečíslování řádků a sloupců matice $A_T$).
Aby při Choleského rozkladu matice $A_T$ nedošlo k žádnému zaplnění, stačí když pomocí topologického číslování očíslujeme vrcholy jí přidruženého grafu (z předpokladu se jedná o strom) a řádky a sloupce matice $A_T$ seřadíme odpovídajícím způsobem. Pak zjevně platí, že matice $A_T$ má, s výjimkou posledního řádku, pod diagonálou vždy právě jeden nenulový prvek.
...
\TODO{Ilustrace teorie pomoci obrazku}

Většinou však nepracujeme s maticemi, jejichž přidružený graf by byl stromem. Zavedeme tedy konstrukci pro libovolnou řídkou, ireducibilní, pozitivně definitní, symetrickou matici $A$ o rozměrech $n \times n$.
...
\begin{definition}
    ...
    Podstrom $T(A)$ s kořenem $x_j$ značíme $T[x_j]$. Množinu vrcholů tohoto stromu značíme taktéž $T[x_j]$.
\end{definition}
