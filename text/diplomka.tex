...
Dělení grafů na~$k$ podgrafů je praktický problém s~bohatým teoretickým zázemím a~mnoha aplikacemi \cite{poth:96}. Může nám pomoci při~řešení parciálních diferenciálních rovnic na~moderních počítačových architekturách \cite{posl:90} a~nezanedbatelnou roli hraje také při výrobě mikroprocesorů metodou VLSI nebo při~řešení velkých systémů lineárních rovnic \cite{keli:70, pis:84}. Často dochází k omezení se na~dělení grafu na~dva podgrafy, v~této práce se však pokusíme prozkoumat i~dělení grafu na~více částí.
...
\begin{enumerate}
  ...
  \item Nalezneme takové $N \in \bigl\{1,\ldots,\min\bigl\{|V_1|,|V_2|\bigr\}\bigr\}$, aby $\sum_{j=1}^{N}g_i$ byla maximální.
...
\begin{enumerate}
  \item $\varphi^{-1}\Bigl(\bigl\{1,\ldots,|C_1|\bigr\} \Bigr)=C_1$
  \item $\varphi^{-1}\Bigl(\bigl\{|C_1|+1,\ldots,|C_1|+|C_2|\bigr\} \Bigr)=C_2$
  \item $\varphi^{-1}\Bigl(\bigl\{|C_1|+|C_2|+1,\ldots,n\bigr\} \Bigr)=S$,
\end{enumerate}
