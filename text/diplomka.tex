...
\label{Cholesky_operace}
  ...
  \[
  \frac12\sum_{i=1}^{N-1}\eta(v_i)\bigl(\eta(v_i)+3\bigr)=
...
\begin{proof}
  ...
  V~$i$-tém kroku algoritmu potřebujeme $\eta(\vec{v_i})$ operací pro~výpočet $v_i/\sqrt{d_i}$ a~$\frac12\eta(\vec{v_i})\bigl(\eta(\vec{v_i})+1\bigr)$ operací pro~vytvoření matice $\frac{\vec{v_i}\vec{v_i}^T}{d_i}$. Tvrzení věty dostaneme sečtením přes všechna $i$.
\end{proof}
