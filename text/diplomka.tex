...
\begin{definition}
Nechť $\mathcal{P} = \{P_1,\ldots,P_n\}$ je rozkladem množiny vrcholů $V$ grafu $G = (V, E)$.
Faktorgrafem grafu $G$ nazveme graf $G / \mathcal{P} = (\mathcal{P}, \mathcal{E_P})$,
kde $(P_i, P_j)\in \mathcal{E_P}$ pro $i,j \in \widehat{n}$, $i \neq j \Leftrightarrow {P_i \cap \mathrm{adj}_G(P_j) \neq \emptyset}$.
\end{definition}
...
Pro reprezentaci ne nutně čtvercové matice $B$ o rozměrech $m\times n$ můžeme také použít bipartitní graf, který je definován následovně:
...
\begin{definition}
  ...
  \end {definition}
  Takovýto bipartitní graf se standardně značí $G=(R,B,E)$
a pokud za jeho pomoci chceme reprezentovat matici $B$, pokládáme $|R|=m$, $|B|=n$ a $E = \{(i,j') \ | \ i \in R, j' \in B, a_{ij} \neq 0 \}$.
